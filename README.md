...
- Expo / React Native приложение для Android, iOS, планшетов и веб-предпросмотра.
- Главная панель ученика в стиле онлайн-школы: план на сегодня, быстрый вход в урок, курс, словарь, домашние задания, тренажер, разговорную практику, профиль и повторение ошибок.
- План курса: 50 тем отображаются как учебная программа со статусом прохождения и лучшим баллом.
- Личный словарь: лексика из всех тем, поиск по английскому слову/русскому переводу, озвучка и быстрый переход к уроку.
- Домашние задания: продолжение рекомендованного урока, повторение слабых фраз и контрольная проверка из 15 заданий.
- Грамматический тренажер: короткие задания B2+ с объяснением на русском и английском.
- Разговорная практика: быстрые голосовые разогревы по текущей теме перед полным диалогом.
...
- 50 уникальных тем.
- В каждой теме генерируется 55 реплик диалога: вопрос, смысловой перевод, подсказка и пример ответа.
- В каждой диалоговой реплике есть перевод вопроса, разбор полезных фраз и 3 правильных варианта ответа.
- Экран урока стал компактнее: есть меню урока и сворачиваемые блоки `Вопрос`, `Мой ответ`, `Фразы`, `3 варианта`, `Совет`.
- В разбор свободного ответа добавлены подсказки по правописанию и типичным грамматическим ошибкам: `I`, `I agree`, `people are`, `doesn't`, `better`, финальная пунктуация.
...
- Голосовые ответы в тесте через `expo-audio`: запись с микрофона, распознавание через сервер распознавания или демонстрационный режим.
- Самостоятельные ответы в диалоге: текстом или голосом, с расшифровкой и обучающим разбором без начисления баллов.
- Иконки приложения, адаптивная Android-иконка, splash image и favicon лежат в `assets/`.
- Локальная авторизация: профиль ученика создается по имени и электронной почте и хранится на устройстве.
...
- нажми `w`, чтобы открыть веб-предпросмотр.

## Как пользоваться

1. Открой приложение.
2. Переключи меню `RU / EN`, если нужно.
3. На главной панели выбери: `План курса`, `Словарь`, `Домашние задания`, `Тренажер`, `Разговоры` или `Темы`.
4. В `Плане курса` открой одну из 50 тем.
5. Пройди диалог: 55 разговорных реплик.
6. Нажимай динамик, чтобы прослушивать английский вопрос или правильные варианты ответа.
7. В блоке `Мой ответ` напиши свой ответ на английском или нажми `Ответить на вопрос голосом`.
8. Изучи блок `Разбор фраз`: перевод, смысл и пример употребления.
9. Сравни свой ответ с тремя правильными вариантами и русскими объяснениями.
10. После сохранения приложение покажет расшифровку, сильную сторону ответа, подсказку и лексику темы.
11. После последней реплики открой тест.
12. В тесте можно выбрать ответ вручную или нажать `Ответить голосом`.
13. Голосовой ответ записывается, распознается и выбирает вариант ответа.
14. После 15 заданий появляется статистика, прогресс и список слабых фраз.
...
Для реального распознавания речи нужен серверный endpoint. В текущей версии backend использует бесплатный локальный `faster-whisper`, поэтому ключи speech-to-text не нужны.
...
Если `EXPO_PUBLIC_STT_ENDPOINT` не задан, включается демонстрационный режим: приложение имитирует распознавание, чтобы можно было проверить UX без сервера.
...
src/content.ts       50 тем, генератор 55 реплик с переводами, подтекстом и объяснениями, генератор 15 тестовых заданий
...
- Перенести локальную авторизацию на production backend: вход по электронной почте и паролю, Google/Apple, восстановление аккаунта.
